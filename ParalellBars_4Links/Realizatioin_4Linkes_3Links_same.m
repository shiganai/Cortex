...
Cut_Off_Freq_Line_Range = find(Cut_Off_Freq_tmp == 4):find(Cut_Off_Freq_tmp == 5);
...
dthWaist_spline = fnint(ddthWaist_spline, mean(dthWaist(Cut_Range(1:2))) * 0.8);
...
% dthRib_spline = fnint(ddthRib_spline, mean(dthRib(Cut_Range(1:2))));
...
thDataArray = FrPB;
MDataArray = [TauHand, TauShoulder, TauWaist, TauRib];
otherDataArray = zeros(size(pHand(:,1)));
...
%{/
figure(29)
plot(Data_Time, rPB_raw - rPB_raw(1) + q0(1), 'DisplayName', 'raw', 'LineStyle', ':', 'LineWidth', 2)
hold on
% plot(Data_Time, rPB_Hand - rPB_Hand(1) + q0(1), 'DisplayName', 'raw\_FromHand', 'LineStyle', '--', 'LineWidth', 2)
...
RMS_rPB = mean((spline(Data_Time, rPB_raw - rPB_raw(1) + q0(1), time) - rPB).^2)
% RMS_rPB = mean((rPB_raw - rPB_raw(1) + q0(1) - spline(time, rPB, Data_Time)).^2)
...
figure(41)
plot(time, dmomentumG, 'DisplayName', '角運動量の微分')
hold on
plot(time, Torque_F_Hand, 'DisplayName', '外力のトルク')
hold off
legend

figure(42)
plot(time, dmomentumG - Torque_F_Hand, 'DisplayName', '角運動量の微分 - 外力のトルク')
hold on
plot(time, TauHand, 'DisplayName', '手首のトルク')
hold off
legend
%}
...
if breakNum == 1
    ...
    FrPB = [FrPB; zeros(size(time_freefall(2:end,1)))];
    TauHand = [TauHand; zeros(size(time_freefall(2:end,1)))];
    TauShoulder = [TauShoulder; zeros(size(time_freefall(2:end,1)))];
    TauWaist = [TauWaist; zeros(size(time_freefall(2:end,1)))];
...
PrPB = FrPB .* drPB;
PthHand = TauHand .* dthHand;
PthShoulder = TauHand .* dthShoulder;
PthWaist = TauWaist .* dthWaist;
...
TauHand_ActivatedRate = zeros(size(time));
TauShoulder_ActivatedRate = zeros(size(time));
TauWaist_ActivatedRate = zeros(size(time));
