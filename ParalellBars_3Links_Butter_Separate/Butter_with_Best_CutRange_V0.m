...
Nichest_Freq = 200 / 2;
Cut_Range = find(Data_Time == 3.88):find(Data_Time == 7.95);
% Cut_Range = find(Data_Time == 4.19):find(Data_Time == 8.5);

Best_V0_Ratio_thHand = 0.6;
Best_V0_Ratio_thShoulder = 0.1;
Best_V0_Ratio_thWaist = -0.7;
Best_V0_Ratio_hand_x = 0.4;
Best_V0_Ratio_hand_y = 1.2;
Best_V0_Ratio_rPB = 1.1;
...
for ii = 1:size(Cut_Off_Freq_tmp, 1)
    [Butter_b, Butter_a] = butter(2, Cut_Off_Freq_tmp(ii) / Nichest_Freq);
    ddrPB_raw_filt_tmp = filtfilt(Butter_b, Butter_a, ddrPB);
    
    RMS_ddrPB(ii) = (mean((ddrPB_raw_filt_tmp - ddrPB).^2))^(1/2);
end
...
% drPB_spline = fnint(ddrPB_raw_spline, drPB_raw(Cut_Range(1)) * 0.95);
% drPB_spline = fnint(ddrPB_raw_spline, mean(drPB_raw(Cut_Range(1:2))));
