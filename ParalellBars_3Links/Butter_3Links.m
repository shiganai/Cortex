...
vHand = [dhand_x_filt, dhand_y_filt];
vShoulder = vHand + rArm * [-sin(thHand_filt+1/2*pi), cos(thHand_filt+1/2*pi)] .* dthHand_filt;
vWaist = vShoulder + rBody * [-sin(thHand_filt+1/2*pi + thShoulder_filt), cos(thHand_filt+1/2*pi + thShoulder_filt)] .* (dthHand_filt + dthShoulder_filt);
vToe = vWaist + rLeg * [-sin(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt), cos(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt)] .* (dthHand_filt + dthShoulder_filt + dthWaist_filt);

pArmMCD = pHand + rArmMCD * [cos(thHand_filt+1/2*pi), sin(thHand_filt+1/2*pi)];
pBodyMCD = pShoulder + rBodyMCD * [cos(thHand_filt+1/2*pi + thShoulder_filt), sin(thHand_filt+1/2*pi + thShoulder_filt)];
pLegMCD = pWaist + rLegMCD * [cos(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt), sin(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt)];

vArmMCD = vHand + rArmMCD * [-sin(thHand_filt+1/2*pi), cos(thHand_filt+1/2*pi)] .* dthHand_filt;
vBodyMCD = vShoulder + rBodyMCD * [-sin(thHand_filt+1/2*pi + thShoulder_filt), cos(thHand_filt+1/2*pi + thShoulder_filt)] .* (dthHand_filt + dthShoulder_filt);
vLegMCD = vWaist + rLegMCD * [-sin(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt), cos(thHand_filt+1/2*pi + thShoulder_filt + thWaist_filt)] .* (dthHand_filt + dthShoulder_filt + dthWaist_filt);
...
T = 1/2 * mPB * vecnorm(vHand, 2, 2).^2 ...
    ...
    + 1/2 * InertiaArm * dthHand_filt.^2 ...
    + 1/2 * InertiaBody * (dthHand_filt + dthShoulder_filt).^2 ...
    + 1/2 * InertiaLeg * (dthHand_filt + dthShoulder_filt + dthWaist_filt).^2;
...
figure(26)
plot(Data_Time(Cut_Range), [T, U, T + U])
legend('T', 'U', 'T + U')
grid on
