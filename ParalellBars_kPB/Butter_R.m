...
for ii = 1:size(Data_Time, 1)
    linePB = spline([BarR0_2D(ii,1), BarR50_2D(ii,1), BarR100_2D(ii,1), BarR130_2D(ii,1), BarR180_2D(ii,1), BarR230_2D(ii,1)],...
        [BarR0_2D(ii,2), BarR50_2D(ii,2), BarR100_2D(ii,2), BarR130_2D(ii,2), BarR180_2D(ii,2), BarR230_2D(ii,2)], linspace(BarR0_2D(ii,1), BarR230_2D(ii, 1), 100));
    
    
    
    linePB = linePB - (BarR0_2D(ii,2) + BarR230_2D(ii,2))/2;
...
%     if linePB(end) < -0.1 * 1e-3
        ...
        plot(linspace(BarR0(ii,1), BarR230(ii, 1), 100), linePB)
    end
end
hold off
...
Bar_Data_All = (dlmread('Trimmed_BarR40kg1.trc','',6,1))/1000;%単位変換[mm]→[m]
...
Bar_Data_All = (dlmread('Trimmed_BarR20kg1.trc','',6,1))/1000;%単位変換[mm]→[m]
